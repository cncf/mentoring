...
- Description: It’s important that non-trivial changes in OpenFeature are agreed upon and documented. The OpenFeature Enhancement Proposal process should be used to gather community feedback and ensure overall alignment with the project. This process should be as streamlined as much as possible to encourage community participation.
...
- Mentor(s): Thomas Schuetz @thschue (thomas.schuetz at t-sc dot eu)
- Expected project size: 175h
- Difficulty: Easy
...
#### Argo Rollouts integration for the Lifecycle Toolkit
- Description: Currently, Keptn (LT) supports Kubernetes Deployments, StatefulSets and Daemonsets for the usage of Pre-/Post-Deployment Tasks and Evaluations. Although these resources might fit for many use cases, Argo Rollouts are also evolving and it would be great to support them as well. The goal of this project is to add Argo Rollouts as a new resource type for the LT.
- Expected outcome: The Keptn (LT) supports Argo Rollouts as a new resource type for the usage of Pre-/Post-Deployment Tasks and Evaluations. Therefore, if a Rollout is used, the Deployment is held back until all Pre-Deployment Tasks are finished and the evaluation is passed. After the Deployment is finished (100%), the Post-Deployment Tasks and Evaluations are executed.
- Recommended Skills: Go, Kubernetes, Argo Rollouts
- Mentor(s): Thomas Schuetz @thschue (thomas.schuetz at t-sc dot eu), Giovanni Liva @giovanniliva (giovanni.liva at dynatrace dot com)
- Expected project size: 175 hours
- Difficulty: Medium
- Upstream Issue (URL): https://github.com/keptn/lifecycle-toolkit/issues/856