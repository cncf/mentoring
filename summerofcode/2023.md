...
- Expected outcome: The Keptn (LT) supports Argo Rollouts as a new resource type for the usage of Pre-/Post-Deployment Tasks and Evaluations. Therefore, if a Rollout is used, the Deployment is held back until all Pre-Deployment Tasks are finished and the evaluation is passed. After the Deployment is finished, the Post-Deployment Tasks and Evaluations are executed.
