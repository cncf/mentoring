...
#### Add Run-Length-Encoding for samples in TSDB

- Description: Prometheus stores the scraped data in it's own [Time Series Database](https://www.wikiwand.com/en/Time_series_database) (TSDB). Due to the nature of often collecting simliar values they can be stored in sequenence using optimized encodings. In Prometheus v2.0 and above all values are encoded using [XOR'ed](https://blog.acolyer.org/2016/05/03/gorilla-a-fast-scalable-in-memory-time-series-database/) int64. Almost since the initial v2.0 release there is a long standing issue on adding other encodings to Prometheus' TSDB. Especially cases where either all values of the series are either `0` or `1` are prime candidates for [Run-Length-Encoding](https://www.wikiwand.com/en/Run-length_encoding) (RLE). After conducting an [experiment on the real world feasbility for adding RLE](https://github.com/prometheus/prometheus/pull/10275), that looks very promising (saving up to 10% disk), we want to work with you to implement RLE in TSDB and improve Prometheus' storage!
- Recommended Skills: Golang
- Mentor(s): Ganesh Vernekar (@codesome), Matthias Loibl (@metalmatze)
- Upstream Issue (URL): https://github.com/prometheus/prometheus/issues/5865
### Thanos
